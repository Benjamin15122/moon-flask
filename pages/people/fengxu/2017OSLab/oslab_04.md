...
我们现有的OSlab实现的是进程，而非线程，因此无法通过全局变量的方式共享信号量；
...
从工作量来看，线程的创建比进程的创建更加简单。现在你可以实现线程匿名信号量了。
...
为了展示你实现的进程间通信，你需要通过你的进程/线程（二选一就可以）间通信方式解决**生产者和消费者问题**。
